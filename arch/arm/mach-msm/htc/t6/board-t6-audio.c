...
static const char *btsco_rate_text[] = {"8000", "16000", "48000"};
static const struct soc_enum msm_btsco_enum[] = {
		SOC_ENUM_SINGLE_EXT(3, btsco_rate_text),
};
static const char * const slim1_tx_ch_text[] = {"One", "Two"};
static const struct soc_enum msm_btsco_tx_ch_enum[] = {
...
static int msm_btsco_tx_ch_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: msm_btsco_tx_ch  = %d\n", __func__,
		 msm_btsco_tx_ch);

	ucontrol->value.integer.value[0] = msm_btsco_tx_ch - 1;
	return 0;
}

static int msm_btsco_tx_ch_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	msm_btsco_tx_ch = ucontrol->value.integer.value[0] + 1;
...
static const struct snd_kcontrol_new tabla_msm_controls[] = {
	...
	SOC_ENUM_EXT("SLIM_1_TX Channels", msm_btsco_tx_ch_enum[0],
		      msm_btsco_tx_ch_get, msm_btsco_tx_ch_put),
				...
				struct snd_pcm_hw_params *params)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		ret = snd_soc_dai_set_channel_map(cpu_dai, msm_btsco_tx_ch,
						  tx_ch, 0, 0);
		if (ret < 0) {
...
static int __init msm_audio_init(void)
{
	...
	rc = gpio_request(AUD_HP_EN_GPIO, "hp_en");
	if (rc) {
		pr_err("%s: Failed to request gpio %d\n", __func__,
			AUD_HP_EN_GPIO);
	}

	rc = pm8xxx_gpio_config(AUD_HP_EN_GPIO, &param);
	if (rc)
