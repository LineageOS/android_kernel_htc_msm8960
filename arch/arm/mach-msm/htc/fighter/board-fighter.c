...
{
	int i = 0, size = ARRAY_SIZE(chg_batt_params);

	if (strcmp(str, "PASS") != 0) {
		pr_info("[BATT] use default HTC battery config\n");
		for (i=0; i < size; i++) {
...
//	create_proc_read_entry("dying_processes", 0, NULL, dying_processors_read_proc, NULL);
	...
	if (properties_kobj) {
		if (!sysfs_create_group(properties_kobj,
			...
			for (i = 0; i < ARRAY_SIZE(syn_ts_3k_data); i++) {
				syn_ts_3k_data[i].vk_obj = properties_kobj;
				syn_ts_3k_data[i].vk2Use = &virtual_keys_synaptics_attr;
			}
		}
	}
	fighter_init_keypad();
