...
static int pmic_regulator_enable(const char *name, unsigned int volt)
{
	...
	pr_info("HTC sleep clk: %s initialized\n", name);
	...
	pr_info("HTC sleep clk: %s enabled\n", name);
	return 0;
