...
struct msm_gpiomux_config dlxp_ethernet_configs[] = {
};
#endif
...
static struct msm_gpiomux_config dlxp_hsic_configs[] = {
	{
		.gpio = 88,               /*HSIC_STROBE */
		.settings = {
...
static struct msm_gpiomux_config dlxp_gsbi_configs[] __initdata = {
	{
		.gpio      = 21,		
		.settings = {
...
static struct msm_gpiomux_config dlxp_slimbus_config[] __initdata = {
	{
		.gpio   = 40,           /* slimbus clk */
		.settings = {
...
static struct msm_gpiomux_config dlxp_audio_codec_configs[] __initdata = {
	{
		.gpio = 39,
		.settings = {
...
void __init dlxp_init_gpiomux(void)
{
	int rc;
...
#if defined(CONFIG_KS8851) || defined(CONFIG_KS8851_MODULE)
	msm_gpiomux_install(dlxp_ethernet_configs,
			ARRAY_SIZE(dlxp_ethernet_configs));
#endif
#if 0
	msm_gpiomux_install(wcnss_5wire_interface,
			ARRAY_SIZE(wcnss_5wire_interface));
#endif
	msm_gpiomux_install(dlxp_gsbi_configs,
			ARRAY_SIZE(dlxp_gsbi_configs));

	msm_gpiomux_install(dlxp_slimbus_config,
			ARRAY_SIZE(dlxp_slimbus_config));

	msm_gpiomux_install(dlxp_audio_codec_configs,
			ARRAY_SIZE(dlxp_audio_codec_configs));
...
#ifdef CONFIG_USB_EHCI_MSM_HSIC
	msm_gpiomux_install(dlxp_hsic_configs,
		ARRAY_SIZE(dlxp_hsic_configs));
#endif
}
