...
#include "board-dlxp-wifi.h"

int dlxp_wifi_power(int on);
int dlxp_wifi_reset(int on);
int dlxp_wifi_set_carddetect(int on);
int dlxp_wifi_get_mac_addr(unsigned char *buf);
...
static void *dlxp_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init dlxp_init_wifi_mem(void)
{
	int i;
...
static unsigned dlxp_wifi_update_nvs(char *str)
{
	unsigned char *ptr;
	unsigned len;
...
int dlxp_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk(KERN_INFO"dlxp_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
		ether_mac_addr[0], ether_mac_addr[1], ether_mac_addr[2], ether_mac_addr[3], ether_mac_addr[4], ether_mac_addr[5]);

	return 0;
}
static struct resource dlxp_wifi_resources[] = {
	[0] = {
...
static struct wifi_platform_data dlxp_wifi_control = {
	.set_power      = dlxp_wifi_power,
	.set_reset      = dlxp_wifi_reset,
	.set_carddetect = dlxp_wifi_set_carddetect,
	.mem_prealloc   = dlxp_wifi_mem_prealloc,
	.get_mac_addr	= dlxp_wifi_get_mac_addr,
};

static struct platform_device dlxp_wifi_device = {
	.name		= "bcmdhd_wlan",
	.id		= 1,
	.num_resources  = ARRAY_SIZE(dlxp_wifi_resources),
	.resource       = dlxp_wifi_resources,
	.dev		= {
	.platform_data = &dlxp_wifi_control,
	},
};

int __init dlxp_wifi_init(void)
{
	int ret;
...
#else
	dlxp_wifi_update_nvs("sd_oobonly=1\n");
#endif
	dlxp_wifi_update_nvs("btc_params80=0\n");
	dlxp_wifi_update_nvs("btc_params6=30\n");
	dlxp_init_wifi_mem();
	ret = platform_device_register(&dlxp_wifi_device);
	return ret;
}
