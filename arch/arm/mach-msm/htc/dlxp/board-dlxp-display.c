...
static int dlxp_detect_panel(const char *name)
{
#if 0
...
static struct msm_fb_platform_data msm_fb_pdata = {
	.detect_client = dlxp_detect_panel,
#ifdef CONFIG_UPDATE_LCDC_LUT
...
void __init dlxp_allocate_fb_region(void)
{
	void *addr;
	unsigned long size;
...
void __init dlxp_mdp_writeback(struct memtype_reserve* reserve_table)
{
...
static struct mipi_dsi_panel_platform_data *mipi_dlxp_pdata;

static struct dsi_buf dlxp_panel_tx_buf;
static struct dsi_buf dlxp_panel_rx_buf;
...
static int dlxp_lcd_on(struct platform_device *pdev)
{
	...
	if(!first_init) {
		if (mipi->mode == DSI_VIDEO_MODE) {
			mipi_dsi_cmds_tx(&dlxp_panel_tx_buf, video_on_cmds,
				video_on_cmds_count);
		}
	}
	first_init = 0;
...
static int dlxp_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
...
static int __devinit dlxp_lcd_probe(struct platform_device *pdev)
{
	if (pdev->id == 0) {
		mipi_dlxp_pdata = pdev->dev.platform_data;
		return 0;
	}
...
static void dlxp_display_on(struct msm_fb_data_type *mfd)
{
...
static void dlxp_display_off(struct msm_fb_data_type *mfd)
{
...
static unsigned char dlxp_shrink_pwm(int val)
{
...
static void dlxp_set_backlight(struct msm_fb_data_type *mfd)
{
	int rc;

	write_display_brightness[2] = dlxp_shrink_pwm((unsigned char)(mfd->bl_level));
...
static struct platform_driver this_driver = {
	.probe  = dlxp_lcd_probe,
	.driver = {
		.name   = "mipi_dlxp",
	},
};

static struct msm_fb_panel_data dlxp_panel_data = {
	.on	= dlxp_lcd_on,
	.off	= dlxp_lcd_off,
	.set_backlight = dlxp_set_backlight,
	.display_on = dlxp_display_on,
	.display_off = dlxp_display_off,
};

static struct msm_panel_info pinfo;
static int ch_used[3] = {0};

static int mipi_dlxp_device_register(struct msm_panel_info *pinfo,
					u32 channel, u32 panel)
{
	...
	pdev = platform_device_alloc("mipi_dlxp", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;

	dlxp_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &dlxp_panel_data,
		sizeof(dlxp_panel_data));
	if (ret) {
...
static int __init dlxp_panel_init(void)
{
	int ret;
	...
	mipi_dsi_buf_alloc(&dlxp_panel_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&dlxp_panel_rx_buf, DSI_BUF_SIZE);
...
device_initcall_sync(dlxp_panel_init);
