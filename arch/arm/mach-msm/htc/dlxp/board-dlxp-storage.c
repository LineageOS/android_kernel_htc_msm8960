...
static struct mmc_platform_data *dlxp_sdc1_pdata = &sdc1_data;
#else
static struct mmc_platform_data *dlxp_sdc1_pdata;
#endif
 ...
 */
static struct embedded_sdio_data dlxp_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int
dlxp_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static int dlxp_wifi_cd;	/* WiFi virtual 'card detect' status */

static unsigned int dlxp_wifi_status(struct device *dev)
{
	return dlxp_wifi_cd;
}

static u32 dlxp_wifi_setup_power(struct device *dv, unsigned int vdd)
{
	/* Dummy function since pads are used for power */
	return 0;
}

static unsigned int dlxp_wifislot_type = MMC_TYPE_SDIO_WIFI;
static unsigned int wifi_sup_clk_rates[] = {
	400000, 24000000, 48000000, 96000000, 192000000
};
static struct mmc_platform_data dlxp_wifi_data = {
	.ocr_mask       = MMC_VDD_28_29,
	.translate_vdd  = dlxp_wifi_setup_power,
	.mmc_bus_width  = MMC_CAP_4_BIT_DATA,
	...
	.status		= dlxp_wifi_status,
	.register_status_notify = dlxp_wifi_status_register,
	.embedded_sdio	= &dlxp_wifi_emb_data,
};

int dlxp_wifi_set_carddetect(int val)
{
...
EXPORT_SYMBOL(dlxp_wifi_set_carddetect);
...
void __init dlxp_init_mmc(void)
{
	wifi_status_cb = NULL;

	printk(KERN_INFO "dlxp: %s\n", __func__);
	...
	apq8064_add_sdcc(1, dlxp_sdc1_pdata);
	apq8064_add_sdcc(3, &dlxp_wifi_data);
