...
int htc_get_wifi_data(char *buf, int count)
{
	unsigned char *ptr;
	unsigned len;

	ptr = get_wifi_nvs_ram();
	// TODO: verify making this a safe copy doesn't break anything
	// original code unconditionally copies NVS_DATA_OFFSET bytes
	len = min((unsigned) count, (unsigned) NVS_DATA_OFFSET);
	memcpy(buf, ptr, len);
	return len;
}
...
static int wifi_data_read_proc(char *page, char **start, off_t off,
					int count, int *eof, void *data)
{
	return htc_get_wifi_data(page, count);
}
