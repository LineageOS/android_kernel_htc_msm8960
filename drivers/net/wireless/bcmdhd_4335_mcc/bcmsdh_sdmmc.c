	...
	if (0 != dma_len) {
		...
		for (pnext = pkt;
			 ...
			 /* HTC_WIFI_START */
			 {
				 void *p;
				 void *q;
				 if (probe_kernel_address( PKTDATA(sd->osh,pnext), p) || probe_kernel_address(PKTDATA(sd->osh,pnext)+PKTLEN(sd->osh,pnext) , q)) { /* something wrong in skb */
...
txglomfail:
			...
			/* HTC_WIFI_START */
			{
				 void *p;
				 void *q;
				 if (probe_kernel_address(buf, p) || probe_kernel_address(buf+pkt_len, q)) { /* something wrong in buf */
